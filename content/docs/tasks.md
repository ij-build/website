...
See the section on [extending a task](/docs/extend#extending-a-task) about the semantics of the `extends` property. It may be of note that the `extends` property does **not** support environment expansion.
...
| healthcheck             |          | {}         | A [healthcheck configuration object](/docs/tasks#healthcheck-configuration). |
...
The file referenced by `export-environment-file` should be formatted like an env file as discussed in the documentation on [environments](/docs/environment#environment). Each relevant line of the file will be added to the working environment set made available to tasks in future stages in the same run.
...
The [environment](/docs/environment#environment) built to execute a plan task is merged into the environment of the target plan. The environment active at the time of this task invocation is inserted after the override environment, but before the environment of a task referenced by the target plan.
